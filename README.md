...
適当なフォルダーを用意し、`tar`コマンドで展開してください。新規に`sabatrapd-dist`フォルダーを作り、ダウンロードフォルダーにあるファイルをそこに展開する例を以下に示します。
...
- SNMP Trapを送る機器上で送信先ポートを指定できないときには、sabatrapd側の監視ポート番号を標準ポートである「162」にします。
...
正常に稼働する`sabatrapd.yml`が用意できたら、インストーラーの`install.sh`を管理者権限で実行してください。
...
実行ファイルと設定ファイルのインストール先フォルダーは環境変数`DESTBINDIR`および`DESTETCDIR`で指定し、`sudo -E ./install.sh`とすれば変更できます。
